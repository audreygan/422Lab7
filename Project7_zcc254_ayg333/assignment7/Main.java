...
public class Main extends Application{
    ...
    public void start(Stage primaryStage) {
        ...
        bc.setTitle("Cheaters!");
        yAxis.setLabel("Number of Matching Phrases");
        xAxis.setLabel("Pairs of Files");

        ArrayList<FilePair> filePairs = graphicsBegin();

        XYChart.Series series = new XYChart.Series();
        for (FilePair f : filePairs) {
            series.getData().add(new XYChart.Data(f.file1 + ", " + f.file2, f.numMatches));
        }
    ...
    /**
     * The model behind the graphics
     * @return the list of file pairs that are above the minimum threshold of common phrases
     */
    private static ArrayList<FilePair> graphicsBegin() {
        ...
        while (input.size() != 0) {                                                     // Keep running if "quit" command not entered
            if (input.size() != 1) {                                                    // If exception was encountered then keep running, not a "quit"
                ...
                return displayGraphicSimilarities(matchesMap);
            }
    ...
    /**
     * The original model for outputting to the console
     */
    ...
    /**
     * This method creates the list that contains the pairs of files with more common phrases than the minimum
     * @param matchesMap is the hashmap with all the phrases and corresponding file pairs
     * @return an arraylist with the filepairs that have similarities above the minimum
     */
    private static ArrayList<FilePair> displayGraphicSimilarities(HashMap<String, FilePair> matchesMap) {
        ArrayList<FilePair> sortedPairs = new ArrayList<>(matchesMap.values());
        ArrayList<FilePair> graphicPairs = new ArrayList<>();
